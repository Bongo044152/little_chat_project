...
void chat_page(ScreenInteractive &screen, const std::string name)
{
    ...
    auto chat_box = Renderer([&] {
        ...
        return window(
            text("Chat History") | bold | hcenter,
            vbox(std::move(lines))
    ...
    // 模擬 Server 發送訊息
    std::thread server([](std::vector<std::string> &chat_history, ScreenInteractive &screen){
...
void add_friend_page(ScreenInteractive &screen, const std::string applicant)
{
    ...
    Component toggle = Toggle(&options, &selected)
        | CatchEvent([&](Event e) {
            if (e.is_mouse() &&
