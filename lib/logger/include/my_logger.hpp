 ...
 * 
 * Provides multi-level logging capabilities with configurable sinks and formatters.
 * 
...
private:
    ...
    // -- member variable -- //
    std::unordered_map<std::string, std::shared_ptr<Logger>> LoggerTable_;  ///< Map of named logger instances
    std::mutex mtx_;                                                        ///< Mutex for thread safety
